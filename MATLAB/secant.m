function [str,root,relative_errors,iter] = secant(f_x,a,b,f_min,max_rel_error,max_iter,iter,relative_errors)
    iter = iter+1;

    next = b - (f_x(b)*(b-a))/(f_x(b)-f_x(a));
    relative_error = abs((next-b)/b);
    ...
    if relative_error*100 <= max_rel_error
        ...
        [str,root,relative_errors,iter] = secant(f_x,a,next,f_min,max_rel_error,max_iter,iter,relative_errors);
    end
